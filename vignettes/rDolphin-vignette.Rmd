...
```
write_plots(file.path(system.file(package = "rDolphin"),"extdata"),profiling_data$final_output,imported_data,profiling_data$useful_data)
```
...
```
View(validation(profiling_data$final_output,profiling_data$alarmmatrix,1)$shownmatrix)

```
shows the fitting error in every signal quantification. And this command:

```
View(validation(profiling_data$final_output,profiling_data$alarmmatrix,3)$shownmatrix)
...
If the user wants to load the information and the plot of these suspicious quantifications, he can do it through the  `load_quantification` function. Run these commands to load the lineshape fitting with most fitting error of 'Valine-2':
```
loaded_quantification=load_quantification(profiling_data$useful_data,imported_data,profiling_data$final_output,list(row=23,col=4),imported_data$ROI_data)
loaded_quantification$plot
loaded_quantification$qualitypar
```

![alt text](https://user-images.githubusercontent.com/21126465/33806333-059d973c-ddc7-11e7-8f56-f4ba5d472764.png)

You can try if repeating this quantification can firther optimize the results, through the `not_automatic_quant` function):
```
updated_profiling_data=not_automatic_quant(imported_data,imported_data$final_output,23,imported_data$ROI_data[3:4,,drop=F],imported_data$useful_data)
```
And compare the results:
```
loaded_updated_quantification=load_quantification(updated_profiling_data$useful_data,imported_data,updated_profiling_data$final_output,list(row=23,col=4),imported_data$ROI_data)
loaded_updated_quantification$plot
loaded_updated_quantification$qualitypar
```

![alt text](https://user-images.githubusercontent.com/21126465/33806332-05828d52-ddc7-11e7-9e01-990307c3287c.png)
...
```
![alt text](https://user-images.githubusercontent.com/21126465/33806410-f9ca64c0-ddc7-11e7-8e75-d3a38669bd24.png)
