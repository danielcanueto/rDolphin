...
```
write_plots(file.path(system.file(package = "rDolphin"),"extdata"),profiling_data$final_output,imported_data,profiling_data$reproducibility_data)
```
...
```
View(validation(profiling_data$final_output,profiling_data$alarm_matrix,1)$shown_matrix)

```
shows the fitting error in every signal quantification. And this command:

```
View(validation(profiling_data$final_output,profiling_data$alarm_matrix,3)$shown_matrix)
