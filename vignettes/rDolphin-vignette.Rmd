...
```
shows the difference between the expected chemical $chemical_shift and the calculated chemical $chemical_shift in every signal quantification.
...
```
And now perform the individual TSP quantification in this first spectrum through the `not_automatic_quant` function:
```
updated_profiling_data=not_automatic_quant(imported_data,imported_data$final_output,1,imported_data$ROI_data[1,,drop=F],imported_data$useful_data)
```
And compare the results:
```
loaded_updated_quantification=load_quantification(updated_profiling_data$useful_data,imported_data,updated_profiling_data$final_output,list(row=1,col=1),imported_data$ROI_data)
