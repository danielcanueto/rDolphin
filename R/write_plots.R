...
write_plots = function(export_path,final_output,reproducibility_data,signals_to_plot=NA) {
  ...
  for (ind2 in signals_to_plot) {
    for (ind in 1:nrow(final_output$quantification)) {
      Xdata=reproducibility_data[[ind]][[ind2]]$Xdata
      if(is.null(Xdata)) next
      Ydata=reproducibility_data[[ind]][[ind2]]$Ydata
      plot_data=reproducibility_data[[ind]][[ind2]]$plot_data
      ROI_profile=reproducibility_data[[ind]][[ind2]]$ROI_profile
      ...
      r=which(make.names(paste(ROI_profile[,4],ROI_profile[,5],sep='_'))==colnames(final_output$quantification)[ind2])
      if (length(r)==0) next
      ...
      p[[ind]]=ggplot() +
        ...
        ) + theme(legend.position = "none", text = element_text(size=5)) + ggtitle(paste(rownames(final_output$quantification)[ind]," - fitting error ",round(final_output$fitting_error[ind,ind2],3)," - signal/area ratio ",round(final_output$signal_area_ratio[ind,ind2],3),sep=''))+
