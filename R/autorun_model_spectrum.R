...
autorun_model_spectrum = function(imported_data) {
  ...
  indicators=matrix(NA,nrow(imported_data$ROI_data),6)
  total_signals_parameters=matrix(NA,nrow(imported_data$ROI_data),7)
  ...
  plotdata2 = data.frame(Xdata=as.numeric(imported_data$ppm),Ydata=rep(0,length(imported_data$ppm)))
  plotdata3 = data.frame(Xdata=as.numeric(imported_data$ppm),Ydata=rep(0,length(imported_data$ppm)))
  ...
  for (ROI_index in seq_along(imported_data$ROI_separator[, 1])) {
    ...
    ROI_profile = ROI_data[imported_data$ROI_separator[ROI_index, 1]:imported_data$ROI_separator[ROI_index, 2],]
    ...
    if (fitting_type == "Clean Sum" ||
      ...
      results_to_save=integration_variables$results_to_save
      ...
      plotdata2$Ydata[ROI_buckets]= integration_variables$plot_data[1,]
      plotdata3$Ydata[ROI_buckets]= integration_variables$plot_data[3,]
      ...
      total_signals_parameters[signals_codes,]=t(signals_parameters[,signals_to_quantify])
      ...
      plotdata3$Ydata[ROI_buckets]=output_data$fitted_sum
  ...
  dummy=list(p=p,indicators=indicators,total_signals_parameters=total_signals_parameters,ROI_data=ROI_data)
  return(dummy)
}
