...
fittingloop_2 = function(FeaturesMatrix,Xdata,Ydata,program_parameters) {
    ...
    if (iterrep <= fitting_maxiterrep& error22 < (program_parameters$additional_signal_improvement * dummy) &
      ...
      if (is.null(residual_peaks$maxtab) == F) {
        ...
        if (nrow(valid_residual_peaks) > program_parameters$signals_to_add) {
          ...
          fg=which(valid_residual_peaks[,2]>0.25*min(signals_parameters[seq(1,length(signals_parameters),5)][signals_to_quantify]))
          valid_residual_peaks=valid_residual_peaks[fg,]
