...
signparpred=function(initial_matrix,fitting_error=NULL,met_names=NA) {
  ...
  predictors=data.frame(initial_matrix[,analyzed_signals],
  prcomp(scale(initial_matrix[,analyzed_signals]))$x[,1:5])
  predictors=predict(caret::preProcess(predictors,c("center","scale","nzv")),predictors)
...
predicted_matrix[,]=lower_bound_matrix[,]=upper_bound_matrix[,]=NA
  ...
  for (i in analyzed_signals) {
    if (is.na(met_names)) {
      idx=which(colnames(predictors)==colnames(initial_matrix)[i])
      training_data=data.frame(y=initial_matrix[,i],predictors[,-idx])
      ...
      tgrid=expand.grid(.mtry = 0.5*length(which(ind==T)),
                        .splitrule="extratrees",
                        .min.node.size=5)
      set.seed(1);model <- tryCatch(caret::train(y ~ .,
                                                  data = training_data[,ind],
                                                  method = "ranger",
                                                  tuneGrid=tgrid,
                                                  trControl = ctrl),
                         error=function(e)
      ...
      tel=cbind(initial_matrix[,setdiff(sed,i)],prcomp(scale(initial_matrix[,sed]))$x[,1])
      training_data=data.frame(y=initial_matrix[,i],scale(tel))
      set.seed(1);boruta_output <- Boruta::Boruta(y ~ ., data=training_data, doTrace=0)
      ind=colnames(training_data) %in%
        c("y",Boruta::getSelectedAttributes(boruta_output, withTentative = TRUE))
      if (!is.null(fitting_error)) {
      ...
      tgrid=expand.grid(.mtry = length(which(ind==T))-1,
                        .splitrule="extratrees",
                        .min.node.size=5)
      set.seed(1);model <- tryCatch(caret::train(y ~ .,
                                                  data = training_data[,ind],
