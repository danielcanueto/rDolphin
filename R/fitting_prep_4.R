...
fitting_prep_4 = function(Xdata,Ydata,initial_fit_parameters,program_parameters,created_baseline,limits_intensity,widthtol) {
  #Created by Daniel Canueto 30/08/2016
  #Preparation of parameters to optimize to achieve the best fitting
  limits_intensity[1,is.na(limits_intensity[1,])]=0
  limits_intensity[1,limits_intensity[1,]<0]=0
  limits_intensity[2,is.na(limits_intensity[2,])]=max(Ydata)
  limits_intensity[2,limits_intensity[2,]>max(Ydata)]=max(Ydata)
  ...
  FeaturesMatrix[1:signals_to_fit, 1] = limits_intensity[1,]
  FeaturesMatrix[1:signals_to_fit, 2] = limits_intensity[2,]
  ...
  FeaturesMatrix[1:signals_to_fit, 4] = initial_fit_parameters$positions +
    initial_fit_parameters$shift_tolerance
  FeaturesMatrix[1:signals_to_fit, 5] = initial_fit_parameters$widths- initial_fit_parameters$widths * widthtol
  FeaturesMatrix[1:signals_to_fit, 6] = initial_fit_parameters$widths + initial_fit_parameters$widths * widthtol
