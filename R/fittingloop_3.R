...
fittingloop_3 = function(FeaturesMatrix,Xdata,Ydata,program_parameters) {
...
for (i in signals_to_quantify)    {
  ...
  bins=c()
    ...
    if(length(sorted_bins$x)>0) bins= sorted_bins$ix[1:which.min(abs(cumsum(sorted_bins$x)-0.9))]
