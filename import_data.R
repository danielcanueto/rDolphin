...
#Other necessary variables
  ...
  jres_path=as.character(import_profile[14, 2])
  try(source(as.character(import_profile[15, 2])),silent=T)

  if (!exists("program_parameters")) program_parameters=fitting_variables()


  repository=as.data.frame(rio::import(as.character(import_profile[11, 2])))
  ...
  if (bruker_path == '' || expno == '' || processingno == '') {
    ...
    params$buck_step = as.numeric(as.character(import_profile[12, 2]))
